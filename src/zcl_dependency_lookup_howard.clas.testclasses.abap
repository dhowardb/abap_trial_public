...
CLASS ltd_cash_provider_injection DEFINITION FOR TESTING.
  PUBLIC SECTION.
    INTERFACES:
      zif_cash_provider_howard PARTIALLY IMPLEMENTED.
ENDCLASS.

CLASS ltd_cash_provider_injection IMPLEMENTATION.
...
CLASS ltc_get_amount DEFINITION FOR TESTING
  ...
  PRIVATE SECTION.
    ...
    DATA: cut TYPE REF TO zcl_dependency_lookup_howard.
    ...
    METHODS:
      ...
      assert_change
        IMPORTING
          i_input  TYPE i
          i_return TYPE zcl_dependency_lookup_howard=>tt_change,
      ...
      compare_table_any_order
        IMPORTING
          i_actual_table    TYPE zcl_dependency_lookup_howard=>tt_change
          i_expected_table  TYPE zcl_dependency_lookup_howard=>tt_change
        RETURNING
...
CLASS ltc_get_amount IMPLEMENTATION.
  ...
  METHOD setup.
    zcl_cash_provider_injector_how=>inject_cash_provider( NEW ltd_cash_provider_injection( ) ).
    cut = NEW #( ).
  ENDMETHOD.
  ...
  METHOD assert_error_changes.
    cl_abap_unit_assert=>assert_equals( act = cut->get_change( i_input )
                                        exp = VALUE zcl_dependency_lookup_howard=>tt_change(
                                                  ( amount = zcl_dependency_lookup_howard=>error_value-c_amount
                                                    type   = zcl_dependency_lookup_howard=>error_value-c_type ) ) ).
  ENDMETHOD.
