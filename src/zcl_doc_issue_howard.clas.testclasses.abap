...
CLASS ltc_get_amount DEFINITION FOR TESTING
  ...
  PRIVATE SECTION.
    ...
    DATA: cut TYPE REF TO zcl_doc_issue_howard.
    ...
    METHODS:
      ...
      assert_change
        IMPORTING
          i_input  TYPE i
          i_return TYPE zcl_doc_issue_howard=>tt_change,
      ...
      compare_table_any_order
        IMPORTING
          i_actual_table    TYPE zcl_doc_issue_howard=>tt_change
          i_expected_table  TYPE zcl_doc_issue_howard=>tt_change
        RETURNING
...
CLASS ltc_get_amount IMPLEMENTATION.
  ...
  METHOD assert_error_changes.
    cl_abap_unit_assert=>assert_equals( act = cut->get_change( i_input )
                                        exp = VALUE zcl_doc_issue_howard=>tt_change(
                                                  ( amount = zcl_doc_issue_howard=>error_value-c_amount
                                                    type   = zcl_doc_issue_howard=>error_value-c_type ) ) ).
  ENDMETHOD.
