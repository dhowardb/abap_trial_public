...
CLASS ltd_backdoor_injection IMPLEMENTATION.
  ...
  METHOD zif_cash_provider_howard~get_notes.
    r_change = VALUE #( ( amount = 1 type = zcl_backdoor_injection_how=>money-coin )
                        ( amount = 5 type = zcl_backdoor_injection_how=>money-coin )
                        ( amount = 4 type = zcl_backdoor_injection_how=>money-coin ) ).
  ENDMETHOD.

ENDCLASS.

CLASS ltc_get_amount DEFINITION DEFERRED.
CLASS zcl_backdoor_injection_how DEFINITION LOCAL FRIENDS ltc_get_amount.
...
CLASS ltc_get_amount DEFINITION FOR TESTING
  ...
  PRIVATE SECTION.
    ...
    DATA: cut TYPE REF TO zcl_backdoor_injection_how.
    ...
    METHODS:
      ...
      assert_change
        IMPORTING
          i_input  TYPE i
          i_return TYPE zcl_backdoor_injection_how=>tt_change,
      ...
      compare_table_any_order
        IMPORTING
          i_actual_table    TYPE zcl_backdoor_injection_how=>tt_change
          i_expected_table  TYPE zcl_backdoor_injection_how=>tt_change
        RETURNING
...
CLASS ltc_get_amount IMPLEMENTATION.
  ...
  METHOD assert_error_changes.
    cl_abap_unit_assert=>assert_equals( act = cut->get_change( i_input )
                                        exp = VALUE zcl_backdoor_injection_how=>tt_change(
                                                  ( amount = zcl_backdoor_injection_how=>error_value-c_amount
                                                    type   = zcl_backdoor_injection_how=>error_value-c_type ) ) ).
  ENDMETHOD.
