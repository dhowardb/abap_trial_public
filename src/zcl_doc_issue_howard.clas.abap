CLASS zcl_doc_issue_howard DEFINITION   "EUR Notes and coins
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_doc_issue_howard IMPLEMENTATION.

  METHOD get_amount_in_coins.
    DATA(provider_cash) = CAST zif_cash_provider_howard( NEW zcl_cash_provider_howard( ) ).
    DATA(notes) = provider_cash->get_notes( 'EUR'  ).
