...
CLASS ltd_setter_injection DEFINITION FOR TESTING.
  PUBLIC SECTION.
  INTERFACES:
    zif_cash_provider_howard PARTIALLY IMPLEMENTED.
ENDCLASS.

CLASS ltd_setter_injection IMPLEMENTATION.

  METHOD zif_cash_provider_howard~get_notes.
    r_change = VALUE #( ( amount = 1 ) ( amount = 5 ) ( amount = 4 ) ).
  ENDMETHOD.
...
CLASS ltc_get_amount DEFINITION FOR TESTING
  ...
  PRIVATE SECTION.
    ...
    DATA: cut TYPE REF TO zcl_setter_injection_how.
    ...
    METHODS:
      ...
      assert_change
        IMPORTING
          i_input  TYPE i
          i_return TYPE zcl_setter_injection_how=>tt_change,
      ...
      compare_table_any_order
        IMPORTING
          i_actual_table    TYPE zcl_setter_injection_how=>tt_change
          i_expected_table  TYPE zcl_setter_injection_how=>tt_change
        RETURNING
...
CLASS ltc_get_amount IMPLEMENTATION.
  ...
  METHOD setup.
    cut = NEW #( ).
    cut->set_cash_provider( i_cash_provider = NEW ltd_setter_injection( ) ).
  ENDMETHOD.
  ...
  METHOD assert_error_changes.
    cl_abap_unit_assert=>assert_equals( act = cut->get_change( i_input )
                                        exp = VALUE zcl_setter_injection_how=>tt_change(
                                                  ( amount = zcl_setter_injection_how=>error_value-c_amount
                                                    type   = zcl_setter_injection_how=>error_value-c_type ) ) ).
  ENDMETHOD.
